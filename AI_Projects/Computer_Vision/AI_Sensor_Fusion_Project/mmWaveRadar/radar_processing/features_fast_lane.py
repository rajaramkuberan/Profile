...
## Defining the region for Fast Lane

vertices_fast = [(-4.5, 3.5), (-0.7, 15), (-5.8, 15), (-7.1, 3.5)]

# Create a Polygon object using the vertices
region_polygon_fast = Polygon(vertices_fast)

# Function to check if a point falls within the defined region
def point_in_region(row):
    point = Point(row["X"], row["Y"])
    return region_polygon_fast.contains(point)

# Filter rows based on whether points fall within the defined region
fast_lane_df_2 = subset_df[subset_df.apply(point_in_region, axis=1)]

## Addding Area and other features

subset_df = fast_lane_df_2
...
fast_lane_input_df = classify_df_ml

def fast_lane_df ():
    """
    Returns the DataFrame containing data related to the fast lane.

    Returns:
        pandas.DataFrame: A DataFrame containing data related to the fast lane.
    """
    global fast_lane_df_2
    return fast_lane_df_2

def fast_lane_features():
    """
    Returns the DataFrame containing features extracted for the fast lane data.

    Returns:
        pandas.DataFrame: A DataFrame containing features extracted for the fast lane data.
    """
    global fast_lane_input_df
    return fast_lane_input_df
